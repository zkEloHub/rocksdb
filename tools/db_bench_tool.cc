...
DEFINE_uint64(level0_column_compaction_trigger_size,
              rocksdb::NvmSetup().Level0_column_compaction_trigger_size, "");
DEFINE_uint64(level0_column_compaction_slowdown_size,
              rocksdb::NvmSetup().Level0_column_compaction_slowdown_size, "");
DEFINE_uint64(level0_column_compaction_stop_size,
              rocksdb::NvmSetup().Level0_column_compaction_stop_size, "");
...
DEFINE_int32(num_column_families, 3, "Number of Column Families to use.");
 ...
 private:
  ...
  void InitializeOptionsGeneral(Options* opts) {
    ...
    if (FLAGS_use_nvm_module && options.nvm_setup == nullptr) {
      ...
      nvm_setup->Level0_column_compaction_trigger_size =
          FLAGS_level0_column_compaction_trigger_size;
      nvm_setup->Level0_column_compaction_slowdown_size =
          FLAGS_level0_column_compaction_slowdown_size;
      nvm_setup->Level0_column_compaction_stop_size =
          FLAGS_level0_column_compaction_stop_size;
  ...
  void OpenDb(Options options, const std::string& db_name,
    ...
    if (FLAGS_num_column_families > 1) {
      printf("run with column_families \n");
      ...
      // }
      ColumnFamilyDescriptor cf_default(ColumnFamilyName(0),
                                        ColumnFamilyOptions(options));
      ColumnFamilyDescriptor cf_meta(ColumnFamilyName(1),
                                     ColumnFamilyOptions(options));
      ColumnFamilyDescriptor cf_info(ColumnFamilyName(2),
                                     ColumnFamilyOptions(options));
      cf_default.options.with_nvm = true;
      cf_info.options.with_nvm = true;
      column_families.push_back(cf_default);
      column_families.push_back(cf_meta);
      column_families.push_back(cf_info);
  ...
  void DoWrite(ThreadState* thread, WriteMode write_mode) {
    ...
    size_t num_key_gens = 1;    // todo: 设为多少?
    if (db_.db == nullptr) {
    ...
    if (FLAGS_num_column_families > 1 && FLAGS_num_hot_column_families > 0) {
      ops_per_stage = (max_ops - 1) / (FLAGS_num_column_families / FLAGS_num_hot_column_families) + 1;
    }
